...
const client = new VulcanHebe(keystore, AccountTools.loadFromObject({ loginId: Number(secrets.loginId), userLogin: secrets.userLogin, userName: secrets.userName, restUrl: secrets.restUrl }));
    ...
    app
        ...
        .get(
            ...
            async ({ query }) => {
                ...
                const grades: Grade[] = await client.getGrades(!Number.isNaN(lastSync.getTime()) ? lastSync : new Date(0));
                return grades;
            },
            {
                query: t.Object({
